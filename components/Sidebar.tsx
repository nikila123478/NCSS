...
import { LayoutDashboard, Wallet, CheckSquare, Users, PenTool, Image, Newspaper, Bell, MessageSquare, LogOut, Home, Shield, ExternalLink, FileText } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab, role }) => {
  ...
  const superAdminItems = [
    ...
    { id: 'all-ids', label: 'All Digital IDs', icon: Shield },
  ...
  return (
    <aside className="fixed inset-y-0 left-0 w-72 glass-panel border-r-0 z-30 pt-24 pb-6 hidden md:block m-4 rounded-3xl h-[calc(100vh-2rem)]">
      <div className="flex flex-col h-full">
        <div className="flex-1 overflow-y-auto py-2 px-4 space-y-1 custom-scrollbar">
          ...
          {/* Generate ID Link (Admin Only) */}
          {(isSuperAdmin || role === 'MEMBER_ADMIN') && (
            <motion.button
              whileHover={{ scale: 1.02, x: 5 }}
          ...
          {isSuperAdmin && (
            <>
              <div className="text-xs font-bold text-gray-500 uppercase px-4 mt-8 mb-2">Administration</div>
